...
//汉化版添加部分
//动态换算游戏内的日元金额，并显示为【XX.XX元】+上方注释【XXX日元】的形式。
//兑换汇率从jpyrate.dbs文件中读取，该文件通过外部exe文件，在每次打开游戏时更新。
command $$yen_calc(property $yen_cost : int)
{
	property $rmb_cost : int
	property $rmb_float : int //鉴于Siglus引擎没有浮点数支持
	property $rmb_int : int //需要手动计算整数部分和小数部分
